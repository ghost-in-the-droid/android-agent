...
struct BGDriveTestView: View {
    ...
    private func run() async {
        ...
        do {
            ...
            var out = ""
            _ = try? await engine.generate(prompt: "Say the word READY and nothing else.", maxTokens: 8) { out += $0 }
            log("BG_INFER_OK '\(out.trimmingCharacters(in: .whitespacesAndNewlines))' @\(elapsed())")
