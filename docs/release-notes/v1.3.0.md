...
- 🧨 **`run_flow` + `chain`** — batched multi-step execution (MCP and agent-loop). One round-trip instead of N. Fail-closed allow-list. Big token/latency win.
- 🧰 **Agent harness primitives** — `screenshot_sequence` + `sub_agent` vision sub-calls, differential a11y diffs, ASCII transliteration, rate-limit backoff + effort-scaled timeouts. Long autonomous runs get cheaper, more legible, more robust.
- ⌨️ **Task-first `ghost` CLI** — `ghost "check reddit" --device asus`. First-run wizard, `ghost mcp install`, and full backwards-compat with `gitd` / `android-agent`.
...
## 62 MCP tools — the full inventory
