...
_Status: RC ready. Blocker list at the bottom._

## What v1.3.0 is

Ghost 1.3 turns Ghost in the Droid from an Android automation harness into a full **agent runtime for real phones**. Six LLM backends (including Gemma running on the phone itself), first-class LangChain and LlamaIndex integrations, a batched MCP execution primitive, one-command install, and a security patch from our first community contributor.

**42 MCP tools** across raw ADB control, screen understanding, skill workflows, batched execution, web search, TTS, camera, and crash diagnostics. Same skill engine. Same phone-as-body model. What changes in 1.3 is the brain — you can now bring any brain, or run one entirely on-device.

## Headline features

- 🧠 **6 LLM backends** — Claude Code (CLI), Anthropic API, OpenRouter, Ollama, on-device Gemma, vLLM. One provider setting. Switch mid-session.
- 📱 **On-device inference** — Gemma runs inside the Android app itself via MediaPipe (`.task`) or llama.cpp (`.gguf`). Airplane-mode works. Nothing leaves the phone.
- 🔗 **LangChain + LlamaIndex adapters** — plug Ghost into your existing agent. Device serial auto-binds; a fail-closed allow-list keeps the exec surface tight.
- 🔐 **`android-agent login`** — bring your Claude Max / Pro subscription. Wraps the sanctioned `claude` CLI auth, no API key needed.
- 🚀 **`uvx ghost-in-the-droid up`** — one-command install + boot. `doctor` runs a preflight; `login` handles auth.
- 🧨 **`run_flow` MCP primitive** — batched multi-step execution. One round-trip instead of N. Fail-closed allow-list. Big token/latency win.
- 🐳 **Docker+KVM emulator backend** — replaces the broken native `avdmanager` path on Linux. First-class emulator fleet management.
- 🔍 **Tracing tab + observability layer** — per-turn traces, token accounting, tool-call visibility, structured logs.
- 🌐 **`web_search` MCP tool** — agent can search mid-conversation without leaving the harness.
- 📷 **`open_camera` + `speak_text`** — first-class camera modes (photo/selfie/video/selfie_video, all timer presets) + on-phone TTS.
- 🩺 **`list_crashes` / `get_crash`** — no-root crash + ANR reports pulled from `logcat -b crash -b events`.
- 🔌 **Marketing jobs seam** — `POST /api/marketing-jobs/enqueue` for external orchestrators.
- 🔐 **Security** — first community security patch shipped (CORS/CWE-352, thanks [@sebastiondev](https://github.com/sebastiondev)).
- 📖 **Docs revamp** — how Ghost compares vs LangChain Agents / OpenAI Agents SDK / Anthropic Computer Use, MCP client compat matrix, LangChain + LlamaIndex code recipes, GSC-indexed sitemap.
...
Point Claude Code, Cursor, Codex, Windsurf, Claude Desktop, or ChatGPT at the local MCP endpoint — same 42 tools, whichever brain you like.

### On-device inference (⭐)

The Ghost companion Android app now hosts a Python runtime (via Chaquopy) with two swappable inference engines:

**MediaPipe path**
- Model format: `.task` (Google's mobile bundle)
- Best for: Gemma 3 1B / 3B / 4B official mobile builds
- ~50 tokens/sec on modern flagship phones for Gemma 3 1B; ~15 tok/s for 3B

**llama.cpp path**
- Model format: `.gguf` (any llama.cpp-compatible quantization)
- Best for: custom fine-tunes, alternative model families, aggressive quantization
- Q4_0 4B models fit under 3GB device RAM

The **model file lives on the phone**, the Kotlin bridge (`OnDeviceLLM.INSTANCE`) exposes an `inference(prompt) → completion` call, Chaquopy talks to it from the Python side, Ghost drives the tool-use loop as if it were any other provider. Airplane mode works end-to-end.

We ship [Ghost-Gemma](https://huggingface.co/ghost-in-the-droid) — a lightly fine-tuned Gemma 3 1B trained specifically on Ghost's flat-shape tool-call format, so on-device tool use is reliable rather than a coin flip. Bring your own model too; the parser accepts both flat and nested shapes.
...
## 42 MCP tools — the full inventory
...
**AndroidWorld** — TBD score, submitted on the unmodified upstream harness (no realigned validators), screenshot + accessibility-tree method, full trajectories submitted for reproducibility. Multiple provider configurations evaluated (Claude 4, Gemma on-device, Ollama).
...
**v1.4** — richer replay + benchmarks + models:
- Visual step-replay report (video + element-tree diff per step)
- MobileWorld submission + more AW config coverage
...
<sup>_Release blockers (internal — remove before publishing):_
